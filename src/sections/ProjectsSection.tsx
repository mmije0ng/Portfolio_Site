...
export function ProjectCard({ project, featured = false, number, onOpen, href, openInNewTab = false, compact = false }: ProjectCardProps) {
  ...
  const isClickable = Boolean(href || (onOpen && project.slug))
  ...
  const cardClassName = cn(
    'group block min-w-0 overflow-hidden rounded-lg border border-sky-200/80 bg-gradient-to-br from-sky-50/74 via-white/40 to-violet-50/68 transition hover:border-sky-300 hover:shadow-[0_18px_40px_rgba(56,189,248,0.2)] backdrop-blur-sm',
  ...
  if (href) {
    return (
      <a className={cardClassName} href={href} rel={openInNewTab ? 'noreferrer' : undefined} target={openInNewTab ? '_blank' : undefined}>
        {cardContent}
      </a>
    )
  }
    ...
    <section className="mx-auto w-full max-w-6xl px-5 py-16 sm:px-8" id="projects-preview">
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {featuredProjects.map((project, index) => (
          <ProjectCard
            featured={index === 0}
            href={papers.some((paper) => paper.slug === project.slug) ? `/research/${project.slug}` : `/projects/${project.slug}`}
            key={project.title}
            number={index + 1}
            openInNewTab
            project={project}
